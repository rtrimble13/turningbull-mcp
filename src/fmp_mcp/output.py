"""FMP-flavored dataset output.

Wraps the shared :mod:`turningbull_mcp.output` writer so callers don't need
to thread ``FMP_OUTPUT_DIR`` through every call site. Tool modules import
:func:`output_dir` to resolve the connector's output location.
"""
...
def output_dir() -> Path:
    """Resolve the FMP-specific output directory (``$FMP_OUTPUT_DIR``)."""
    return resolve_output_dir("FMP_OUTPUT_DIR", "./fmp_output")
